...
// Captions read from file metadata, cached per file until it changes
// (size/mtime), so a gallery of hundreds of photos isn't re-parsed on
// every page view. filePath -> { size, mtimeMs, value }
...
async function readEmbeddedDescription(filePath, ext) {
  ...
  const value = await parseEmbeddedDescription(filePath);
  descriptionCache.set(filePath, { size: stat.size, mtimeMs: stat.mtimeMs, value });
  return value;
}

async function parseEmbeddedDescription(filePath) {
  try {
