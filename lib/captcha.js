 ...
 * signed, tamper-proof token carrying a random salt, an expiry time and
 * the proof-of-work difficulty the client must satisfy. The expected
 * answer is NOT in the token (it is only mixed into the signature), so it
 * can't be read from the page - only from the distorted question image.
...
function createChallenge() {
  ...
  const payload = `${salt}:${expires}:${POW_DIFFICULTY_BITS}`;
  const token = Buffer.from(`${payload}:${sign(`${answer}:${payload}`)}`).toString("base64");
...
function verifyChallenge(token, submittedAnswer, nonce) {
  ...
  try {
    ...
    const [salt, expires, difficulty, signature] = parts;
    // The answer only exists inside the signature: recompute it with the
    // submitted answer (normalized, so " 09" still means 9).
    const answer = String(submittedAnswer == null ? "" : submittedAnswer).trim();
    if (!/^\d{1,3}$/.test(answer)) return false;
    const payload = `${salt}:${expires}:${difficulty}`;
    const expectedSignature = sign(`${Number(answer)}:${payload}`);
